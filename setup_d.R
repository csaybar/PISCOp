...
source(sprintf("%s/code/runme_d.R",working_directory))
...
#base son las extensiones que el dr. Waldo propuso a 0.1°.
base<-raster(sprintf("%s/base-inputs/base.tif",database_directory))
...
load(sprintf("%s/base-inputs/daily/collocated_daily.Rdata",database_directory)) #integracion base pisco y sofia
load(sprintf("%s/base-inputs/daily/cutoff_day.Rdata",database_directory)) #grupos e inputs para realizar cutoff
load(sprintf("%s/base-inputs/model_Qm.Rdata",database_directory)) #Modelos de distribucion empirica de cada estacion
ratio_Pd <- brick(sprintf("%s/inputs/Ratios/ratios_PISCOp.nc",database_directory))
