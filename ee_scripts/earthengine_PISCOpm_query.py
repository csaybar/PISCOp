...
end_date = "2018-06-30"
seq_date = pd.date_range(start = start_date, end = end_date, freq = 'MS').tolist()
# -----

PISCOpm = ee.ImageCollection("users/csaybar/PISCOpm")\
            .filterDate(start_date, end_date)\
            .map(my_reduce)
 
# Method - 01
# Limited - Use it just small queries 
ts_pisco = PISCOpm.getInfo()
pisco_values = [x['properties']['value'] for x in ts_pisco['features']]
pisco_pd = pd.DataFrame(data=pisco_values,index=seq_date)
plt.plot(pisco_pd)
plt.show()
...
pisco_task = Export.table.toDrive(**{'collection': PISCOpm,
